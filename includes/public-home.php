...
if(isset($_POST['rordb_claim'])){
    // check permission
    if(current_user_can('rordbv2_claim')) rordbv2_wpdb_claim_item($_POST['rordb_id'], $_POST['rordb_claim']);
}
...
if(isset($_GET['rordb_group'])) $where .= " AND IT.claimedby='".$_GET['rordb_group']."'";
...
foreach($items as $i){
    ...
    <div class='wp-block-columns' style='background-color:#d6c2d6'>
        ...
        <div class='wp-block-column' style='flex-basis:50%;'>
            <b><?php echo $i->name; ?></b>
            <?php 
                // check for permission to edit file
                if(current_user_can('rordbv2_edit_items')){ echo "<a href='?page_id=$pageid&rordb_action=rordb_edit&rordb_edit=item&rordb_id=$i->id'>edit</a>"; }
                // check if permission to claim
                if(current_user_can('rordbv2_claim')){ 
